...
int ConePy::PyInit(PyObject* args, PyObject* kwds)
{
    ...
    static const std::array<const char*, 5>
        keywords_pprr {"Point1", "Point2", "Radius1", "Radius2", nullptr};
    PyErr_Clear();
    if (Base::Wrapped_ParseTupleAndKeywords(
        ...
        GC_MakeConicalSurface mc(gp_Pnt(v1.x, v1.y, v1.z), gp_Pnt(v2.x, v2.y, v2.z), radius1, radius2);
        if (!mc.IsDone()) {
...
{
    Handle(Geom_ConicalSurface) s = Handle(Geom_ConicalSurface)::DownCast(getGeomConePtr()->handle());
    gp_Pnt loc = s->Apex();
