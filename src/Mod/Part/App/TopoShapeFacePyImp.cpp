...
int TopoShapeFacePy::PyInit(PyObject* args, PyObject* /*kwd*/)
{
    ...
    if (PyArg_ParseTuple(
            args,
            "O!O!",
            &(Part::TopoShapeFacePy::Type),
            &face,
            &(Part::TopoShapeWirePy::Type),
            &wire
        )) {
        try {
...
spine = Part.Face(Part.makePolygon([v(0.,0.,0.), v(100.,0.,0.), v(100.,100.,0.), v(0.,100.,0.),
v(0.,0.,0.)])) evolve = spine.makeEvolved(Profile=profile, Join=PartEnums.JoinType.Arc)
*/
...
Py::Object TopoShapeFacePy::getSurface() const
{
    ...
    switch (adapt.GetType()) {
        case GeomAbs_Plane: {
            ...
            Handle(Geom_Plane) this_surf = Handle(Geom_Plane)::DownCast(plane->handle());
