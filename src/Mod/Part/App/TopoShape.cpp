...
unsigned int TopoShape::getMemSize() const
{
    if (!_Shape.IsNull()) {
        ...
        unsigned int memsize = (sizeof(TopoDS_Shape) + sizeof(TopoDS_TShape))
            * TopoShape_RefCountShapes(_Shape);
...
) const
{
    ...
    BRepLib::BuildCurves3d(
        ...
        14,
        10000
    );  // precision must be reduced for very large parts to avoid #17113
    return TopoDS_Shape(std::move(wire));
}
    ...
    if (countShapes < 2) {
        ...
        if (isClosed) {
            ...
            if (profiles.Last().ShapeType() == TopAbs_VERTEX) {
                Base::Console()
                    .message("TopoShape::makeLoft: can't close Loft with Vertex as last profile. 'Closed' ignored.\n");
            }
            else {
...
TopoDS_Shape TopoShape::makeOffsetShape(
    double offset,
    double tol,
    bool intersection,
    bool selfInter,
...
TopoDS_Shape TopoShape::makeOffset2D(
    double offset,
    short joinType,
    bool fill,
...
) const
{
    ...
    if (!shapesToProcess.empty()) {
        ...
        TopoDS_Compound compoundSourceWires;
        {
            ...
            if (haveFaces) {
                // extract plane from first face (useful for preserving the plane of face precisely
                // if dealing with only one face)
        ...
        if (!fill) {
            ...
            if (!allowOpenResult || openWires.empty()) {
                ...
                // check
                if (v1.IsNull()) {
                    throw NullShapeException("v1 is null");
                }
                if (v2.IsNull()) {
                    throw NullShapeException("v2 is null");
                }
                if (v3.IsNull()) {
                    throw NullShapeException("v3 is null");
                }
                if (v4.IsNull()) {
                    throw NullShapeException("v4 is null");
                }
...
TopoDS_Shape TopoShape::makeThickSolid(
...
void TopoShape::
    getFaces(std::vector<Base::Vector3d>& aPoints, std::vector<Facet>& aTopo, double accuracy, uint16_t /*flags*/) const
{
    if (this->_Shape.IsNull()) {
        return;
    }
...
void TopoShape::setFaces(
    const std::vector<Base::Vector3d>& Points,
    const std::vector<Facet>& Topo,
    double tolerance
)
{
    gp_XYZ p1, p2, p3;
...
void TopoShape::getLinesFromSubShape(
...
) const
{
    ...
    for (TopExp_Explorer xp(_Shape, TopAbs_EDGE, TopAbs_FACE); xp.More(); xp.Next()) {
        ...
        GCPnts_UniformAbscissa
            discretizer(curve, lateralDistance, curve.FirstParameter(), curve.LastParameter());
...
void TopoShape::getLinesFromSubElement(
...
#define _HANDLE_NULL_SHAPE(_msg, _throw) \
    do { \
        if (_throw) { \
