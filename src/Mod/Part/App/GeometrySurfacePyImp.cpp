...
{
    GeomAbs_Shape c = Handle(Geom_Surface)::DownCast(getGeometryPtr()->handle())->Continuity();
    std::string str;
    switch (c) {
        case GeomAbs_C0:
            str = "C0";
...
PyObject* GeometrySurfacePy::toBSpline(PyObject* args, PyObject* kwds) const
{
    ...
    static const std::array<const char*, 8> kwlist {
        "Tol3d",
        "UContinuity",
        "VContinuity",
        "MaxDegreeU",
...
PyObject* GeometrySurfacePy::intersectSS(PyObject* args) const
{
    ...
    try {
        if (!surf1.IsNull()) {
            ...
            Handle(Geom_Surface) surf2 = Handle(Geom_Surface)::DownCast(
