...
# endif
    ...
    error = FT_Set_Char_Size(
        ...
        48 * 64 * 10,
        /* char_height in 1/64th of points */  // increased 10X to preserve very small details
    ...
    scalefactor = (stringheight / float(FTFace->height))
        / 10;  // divide scale by 10 to offset the 10X increased scale in FT_Set_Char_Size above
    for (i = 0; i < length; i++) {
        currchar = PyUString[i];
        error = FT_Load_Char(FTFace, currchar, FTLoadFlags);
        if (error) {
...
)
{
    ...
    error = FT_Outline_Decompose(&FTFace->glyph->outline, &FTcbFuncs, &ctx);
    if (error) {
    ...
    int wCount = 0;
    for (std::vector<TopoDS_Wire>::iterator iWire = ctx.Wires.begin(); iWire != ctx.Wires.end();
         ++iWire, wCount++) {
...
int calcClockDir(std::vector<Base::Vector3d> points)
{
    ...
    double area = points.back().x * points.front().y - points.front().x * points.back().y;
    bigArea += area;
    if (bigArea < 0) {
