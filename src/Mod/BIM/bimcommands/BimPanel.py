...
class Arch_Panel:
    ...
    def taskbox(self):
        ...
        self.vHeight.setText(
            FreeCAD.Units.Quantity(self.Thickness, FreeCAD.Units.Length).UserString
        )
    ...
    def setPreset(self, i):
        ...
        if i > 0:
            self.vLength.setText(
                FreeCAD.Units.Quantity(float(Presets[i][1]), FreeCAD.Units.Length).UserString
            )
            self.vWidth.setText(
                FreeCAD.Units.Quantity(float(Presets[i][2]), FreeCAD.Units.Length).UserString
            )
            self.vHeight.setText(
                FreeCAD.Units.Quantity(float(Presets[i][3]), FreeCAD.Units.Length).UserString
            )
        ...
        if FreeCADGui.Selection.getSelection():
            ...
            for obj in FreeCADGui.Selection.getSelection():
                if Draft.getType(obj) == "Panel":
                    FreeCADGui.doCommand(
                        "Arch.makePanelCut(FreeCAD.ActiveDocument." + obj.Name + ")"
                    )
