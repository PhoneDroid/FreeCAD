...
# ***************************************************************************
# *                                                                         *
# SPDX-FileCopyrightText: 2011 Yorik van Havre <yorik@uncreated.net>
# *                                                                         *
...
def read(filename, skip=None):
    ...
    if getIfcOpenShell() and not FORCE_PYTHON_PARSER:
        ...
        if IFCOPENSHELL5:
            ...
            relations = (
                ifc.by_type("IfcRelAggregates")
                + ifc.by_type("IfcRelContainedInSpatialStructure")
                + ifc.by_type("IfcRelVoidsElement")
            )
            if not objects:
            ...
            for c in comps:
                ...
                if parent and (id in ifcObjects):
                    if ifcObjects[id] and (ifcObjects[id].Name != parent.Name):
                        if additive:
                            if DEBUG:
                                print("adding ", ifcObjects[id].Name, " to ", parent.Name)
                            ArchCommands.addComponents(ifcObjects[id], parent)
                        else:
                            if DEBUG:
                                print("removing ", ifcObjects[id].Name, " from ", parent.Name)
        ...
        FreeCAD.Console.PrintWarning(
            translate(
                "Arch", "IfcOpenShell not found or disabled, falling back on internal parser."
            )
            + "\n"
        )
        schema = getSchema()
        if schema:
            ...
            FreeCAD.Console.PrintWarning(
                translate("Arch", "IFC Schema not found, IFC import disabled.") + "\n"
            )
            return None
        t2 = time.time()
        if DEBUG:
            print("Successfully loaded", ifc, "in %s s" % ((t2 - t1)))
    ...
    t3 = time.time()
    if DEBUG:
        print("done processing IFC file in %s s" % ((t3 - t1)))
...
def makeStructure(entity, shape=None, ifctype=None, name="Structure"):
    ...
    try:
        if shape:
            ...
            if DEBUG:
                print(
                    "    made structure object  ", entity, ":", structure, " (type: ", ifctype, ")"
                )
            return structure
        ...
        try:
            ...
            if SEPARATE_OPENINGS and hasattr(IfcImport, "DISABLE_OPENING_SUBTRACTIONS"):
                if SEPARATE_PLACEMENTS and hasattr(IfcImport, "DISABLE_OBJECT_PLACEMENT"):
                    brep_data = IfcImport.create_shape(
                        obj,
                        IfcImport.DISABLE_OPENING_SUBTRACTIONS
                        | IfcImport.DISABLE_OBJECT_PLACEMENT
                        | ss,
                    )
                else:
                    brep_data = IfcImport.create_shape(
                        obj, IfcImport.DISABLE_OPENING_SUBTRACTIONS | ss
                    )
            else:
                if SEPARATE_PLACEMENTS and hasattr(IfcImport, "DISABLE_OBJECT_PLACEMENT"):
            ...
            for g in groups:
                if g[2]:
                    if g[0] in ["Building", "Floor"]:
                        ...
                        fcg = FreeCAD.ActiveDocument.addObject(
...
def export(exportList, filename):
    ...
    for obj in objectslist:
        ...
        if DEBUG:
            print("Adding " + obj.Label + " as Ifc" + ifctype)
            ...
            if not ifctype in supportedIfcTypes:
                if DEBUG:
                    print(
                        "   Type ",
                        ifctype,
                        " is not supported yet. Exporting as IfcBuildingElementProxy instead",
                    )
                ifctype = "IfcBuildingElementProxy"
                extra = ["ELEMENT"]

            product = ifc.addProduct(
                ifctype,
                representation,
                storey=parent,
                placement=placement,
    ...
    if unprocessed:
        print(
            "\nWARNING: "
            + str(len(unprocessed))
            + " objects were not exported (stored in importIFC.unprocessed):"
        )
    ...
    if hasattr(obj, "Proxy"):
        if hasattr(obj.Proxy, "getProfiles"):
            ...
            if (len(p) == 1) and v:
                ...
                d = [
                    r.Base,
                    DraftVecUtils.rounded(r.Rotation.multVec(FreeCAD.Vector(1, 0, 0))),
                    DraftVecUtils.rounded(r.Rotation.multVec(FreeCAD.Vector(0, 0, 1))),
                ]
                # r = r.inverse()
...
class IfcWriter(object):
    ...
    def __init__(
        ...
        owner="",
        organization="",
        application="Python IFC exporter",
        version="0.0",
    ):
        ...
        self._person = create(
            self._fileobject, "IfcPerson", [None, None, "", None, None, None, None, None]
        )
        ...
        self._owner = create(
            self._fileobject, "IfcOwnerHistory", [pno, app, None, "ADDED", None, pno, app, now()]
        )
        ...
        dim9 = create(
            self._fileobject, "IfcConversionBasedUnit", [dim1, "PLANEANGLEUNIT", "DEGREE", dim8]
        )
    ...
    def addGroup(self, entities, name="Default group", description=None):
        ...
        if not isinstance(entities, list):
            entities = [entities]
        gro = create(
            self._fileobject, "IfcGroup", [uid(), self._owner, str(name), description, None]
        )
        rel = create(
            self._fileobject,
    ...
    def addProfile(self, ifctype, data, curvetype="AREA"):
        ...
        if ifctype == "IfcPolyline":
            ...
            profile = create(
                self._fileobject, "IfcArbitraryClosedProfileDef", [curvetype, None, pol]
            )
            ...
            for curve in data:
                ...
                if curve[0] == "line":
                    pts = [
                        create(self._fileobject, "IfcCartesianPoint", getTuple(p)[:2])
                        for p in curve[1]
                    ]
                ...
                if cur:
                    seg = create(
                        self._fileobject, "IfcCompositeCurveSegment", ["CONTINUOUS", True, cur]
                    )
                    curves.append(seg)
            ccu = create(self._fileobject, "IfcCompositeCurve", [curves, False])
            profile = create(
                self._fileobject, "IfcArbitraryClosedProfileDef", [curvetype, None, ccu]
            )
        else:
