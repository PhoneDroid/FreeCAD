...
def insert(srcfile, docname, skip=[], only=[], root=None, preferences=None):
    ...
    for product in products:
        ...
        if hasattr(product, "Representation") and hasattr(
            ...
            if len(product.Representation.Representations) > 0:
                ...
                if len(lays) > 0:
                    ...
                    if preferences["DEBUG"]:
                        print(" layer ", layer_name, " found", ptype, end="")
                else:
                    if preferences["DEBUG"]:
                        print(" no layer found", ptype, end="")
        ...
        if psets and params.get_param_arch("IfcImportFreeCADProperties"):
            if "FreeCADPropertySet" in [ifcfile[pset].Name for pset in psets.keys()]:
                if preferences["DEBUG"]:
                    print(" restoring from parametric definition...", end="")
        ...
        try:
            ...
            if preferences["DEBUG"]:
                print(" ERROR unable to get object representation", end="")
        if (
            prepr
            and (preferences["MERGE_MODE_ARCH"] == 0)
            and archobj
            and preferences["CREATE_CLONES"]
        ):
        ...
        if brep:
            if preferences["DEBUG"]:
                print(" " + str(int(len(brep) / 1000)) + "k ", end="")
            ...
            shape.scale(
                1000.0
            )  # IfcOpenShell always outputs in meters, we convert to mm, the freecad internal unit

            if shape.isNull() and (not preferences["ALLOW_INVALID"]):
                if preferences["DEBUG"]:
                    print("null shape ", end="")
            elif not shape.isValid() and (not preferences["ALLOW_INVALID"]):
                if preferences["DEBUG"]:
                    print("invalid shape ", end="")
            else:
                ...
                if (preferences["MERGE_MODE_ARCH"] > 0 and archobj) or structobj:
                    ...
                    if ptype == "IfcSpace":  # do not add spaces to compounds
                        if preferences["DEBUG"]:
                            print("skipping space ", pid, end="")
                    elif structobj:
                        structshapes[pid] = shape
                        if preferences["DEBUG"]:
                            print(len(shape.Solids), "solids ", end="")
                        baseobj = shape
                    else:
                        shapes[pid] = shape
                        if preferences["DEBUG"]:
                        ...
                        if preferences["GET_EXTRUSIONS"] and (preferences["MERGE_MODE_ARCH"] != 1):
                            ...
                            if product.Representation:
                                if product.Representation.Representations:
                                    if product.Representation.Representations[0].is_a(
                                        "IfcShapeRepresentation"
                                    ):
                                        if product.Representation.Representations[0].Items:
                                            if (
                                                product.Representation.Representations[0]
                                                .Items[0]
                                                .is_a("IfcExtrudedAreaSolid")
                                            ):
                                                profileid = (
                                                    product.Representation.Representations[0]
                                                    .Items[0]
                                                ...
                                                sortmethod = importIFCHelper.getProfileCenterPoint(
                                                    product.Representation.Representations[0].Items[
                                                        0
                                                    ]
                                                )
                            ...
                            if ex:
                                ...
                                if profileid and (profileid in profiles):
                                    ...
                                    r = FreeCAD.Rotation(
                                        baseface.Shape.Faces[0].normalAt(0, 0),
                                        ex[0].Faces[0].normalAt(0, 0),
                                    )
                                    if r.Angle > 0.000001:
                                        # use shape methods to easily obtain a correct placement
                                        ts = Part.Shape()
                                        ts.rotate(
                                            DraftVecUtils.tup(baseface.Shape.CenterOfMass),
                                            DraftVecUtils.tup(r.Axis),
        ...
        if preferences["MERGE_MODE_ARCH"] == 0 and archobj:
                ...
                if clone:
                    ...
                    if shape:
                        ...
                        if hasattr(s1, "CenterOfMass") and hasattr(s2, "CenterOfMass"):
                            ...
                            if product.Representation:
                                r = importIFCHelper.getRotation(
                                    product.Representation.Representations[0].Items[0].MappingTarget
                                )
                                if not r.isNull():
                    ...
                    if (
                        freecadtype in ["Wall", "Structure"]
                        and baseobj
                        and baseobj.isDerivedFrom("Part::Extrusion")
                    ):
            ...
            try:
                if hasattr(obj, "IfcType"):
                    obj.IfcType = "".join(map(lambda x: x if x.islower() else " " + x, ptype[3:]))[
                        1:
                    ]
            except Exception:
            ...
            if hasattr(obj, "IfcData"):
                ...
                for attribute in ArchIFCSchema.IfcProducts[product.is_a()]["attributes"]:
                    ...
                    if (
                        hasattr(product, attribute["name"])
                        and getattr(product, attribute["name"])
                        and hasattr(obj, attribute["name"])
                    ):
        ...
        elif (preferences["MERGE_MODE_ARCH"] == 1 and archobj) or (
            preferences["MERGE_MODE_STRUCT"] == 0 and not archobj
        ):
            ...
            if ptype in ["IfcSite", "IfcBuilding", "IfcBuildingStorey"]:
                ...
                try:
                    if hasattr(obj, "IfcType"):
                        obj.IfcType = "".join(
                            map(lambda x: x if x.islower() else " " + x, ptype[3:])
                        )[1:]
                except Exception:
                    print("Unable to give IFC type ", ptype, " to object ", obj.Label)
                if hasattr(obj, "IfcData"):
        ...
        elif (preferences["MERGE_MODE_ARCH"] == 2 and archobj) or (
            preferences["MERGE_MODE_STRUCT"] == 1 and not archobj
        ):
        ...
        if preferences["DEBUG"]:
            print("")  # newline for debug prints, print for a new object should be on a new line
        ...
        if obj:
                ...
                if preferences["IMPORT_PROPERTIES"] and hasattr(obj, "IfcProperties"):
                    ...
                    if isinstance(obj.IfcProperties, dict):
                        ...
                        obj.addProperty(
                            ...
                            "IfcProperties",
                            "Component",
                            "Stores IFC properties as a spreadsheet",
                            locked=True,
                        )
                    ...
                    for c in psets.keys():
                        o = ifcfile[c]
                        if preferences["DEBUG"]:
                            print("propertyset Name", o.Name, type(o.Name))
                        catname = o.Name
                        for p in psets[c]:
                            l = ifcfile[p]
                            lname = l.Name
                                ...
                                if l.NominalValue:
                                    if preferences["DEBUG"]:
                                        print(
                                            ...
                                            l.NominalValue.is_a(),
                                            type(l.NominalValue.is_a()),
                                        )
                                        print(
                                            "property NominalValue.wrappedValue",
                                            l.NominalValue.wrappedValue,
                                            type(l.NominalValue.wrappedValue),
                                        )
                                    ...
                                    if l.NominalValue.is_a() in [
                                        ...
                                        ifc_spreadsheet.set(
                                            str("D" + str(n)),
                                            "'" + str(l.NominalValue.wrappedValue),
                                        )
                                    else:
                                        ifc_spreadsheet.set(
                                            str("D" + str(n)), str(l.NominalValue.wrappedValue)
                                        )
                                    if hasattr(l.NominalValue, "Unit"):
                                        ifc_spreadsheet.set(
                                            str("E" + str(n)), str(l.NominalValue.Unit)
                                        )
                                n += 1
            ...
            if product.is_a("IfcSite"):
                ...
                modelRC = next(
                    (rc for rc in project.RepresentationContexts if rc.ContextType == "Model"), None
                )
    ...
    if preferences["MERGE_MODE_STRUCT"] == 2:
        ...
        if preferences["DEBUG"]:
            print("Joining Structural shapes...", end="")
        ...
        if preferences["DEBUG"]:
            print("Processing Struct relationships...", end="")
        ...
        for host, children in groups.items():
            if ifcfile[host].is_a("IfcStructuralAnalysisModel"):
                ...
                if host in objects:
                    ...
                    if cobs:
                        if preferences["DEBUG"]:
                            print("adding ", len(cobs), " object(s) to ", objects[host].Label)
    ...
    for host, children in groups.items():
        if ifcfile[host].is_a("IfcGroup"):
            if ifcfile[host].Name:
                ...
                if preferences["DEBUG"]:
                    print(
                        "no group name specified for entity: #",
                        ifcfile[host].id(),
                        ", entity type is used!",
                    )
    ...
    if preferences["MERGE_MODE_ARCH"] == 3:
        ...
        if preferences["DEBUG"]:
            print("Processing Arch relationships...", end="")
        first = True
            ...
            for subtraction in subtractions:
                if (subtraction[0] in objects) and (subtraction[1] in objects):
                    ...
                    if preferences["DEBUG"]:
                        print(
                            "    subtracting",
                            objects[subtraction[0]].Label,
                            "from",
                            objects[subtraction[1]].Label,
                        )
        ...
        for host, children in additions.items():
            ...
            for child in children:
                if (
                    child in objects and child not in swallowed
                ):  # don't add objects already in groups
                    cobs.append(objects[child])
            if not cobs:
                continue
            if preferences["DEBUG"] and first:
            ...
            if (
                ...
                and (
                    not (
                        Draft.getType(objects[host])
                        in ["Site", "Building", "Floor", "BuildingPart", "Project"]
                    )
                )
            ):
                # avoid huge fusions
    ...
    for annotation in annotations:
        ...
        if preferences["DEBUG"]:
            print(
                count, "/", len(annotations), "object #" + str(aid), ":", annotation.is_a(), end=""
            )
        ...
        if preferences["DEBUG"]:
            print("")  # add newline for 2D objects debug prints

    doc.recompute()

    # Materials

    if preferences["DEBUG"] and materials:
        print("Creating materials...", end="")
    # print("\n")
    # print("colors:",colors)
    ...
    for material in materials:
        ...
        if mat_color is not None:
            ...
            if preferences["DEBUG"]:
                print("/n  no color for material: {}, ".format(str(material.id)), end="")
        ...
        if preferences["MERGE_MATERIALS"]:
            for added_mat in added_mats:
                if (
                    "Description"
                    in added_mat.Material  # Description has been set thus it is in mdict
                    ...
                    if (
                        ...
                        or (
                            "DiffuseColor" not in added_mat.Material and "DiffuseColor" not in mdict
                    ...
                    if hasattr(objects[o], "Material"):
                        ...
                        if FreeCAD.GuiUp:
                            ...
                            if ma_color != sh_color:
                                ...
                                print(
                                    "    obj: ", o, "label: ", objects[o].Label, " col: ", sh_color
                                )
    ...
    if preferences["DEBUG"] and layers:
    ...
    if filesize:
        _msg(
            "Finished importing {0} MB "
            "in {1} seconds, or {2} s/MB".format(
                round(filesize, 1), int(endtime), int(endtime / filesize)
            )
        )
    else:
