...
def getIndices(obj, shape, offsetv, offsetvn):
    ...
    if mesh:
        for v in mesh.Topology[0]:
            vlist.append(
                " " + str(round(v[0], p)) + " " + str(round(v[1], p)) + " " + str(round(v[2], p))
            )

        for vn in mesh.Facets:
            vnlist.append(
                " "
                + str(round(vn.Normal[0], p))
                + " "
                + str(round(vn.Normal[1], p))
                + " "
                + str(round(vn.Normal[2], p))
            )
        ...
        for v in shape.Vertexes:
            vlist.append(
                " " + str(round(v.X, p)) + " " + str(round(v.Y, p)) + " " + str(round(v.Z, p))
            )
        if not shape.Faces:
            for e in shape.Edges:
...
def export(exportList, filename, colors=None):
    ...
    for obj in objectslist:
        ...
        if obj.isDerivedFrom("App::Link"):
            ...
            if parents:
                doc = obj.Document
                parents = [parents[0][0]] + [
                    doc.getObject(name) for name in parents[0][1].split(".")[:-2]
                ]
        ...
        if FreeCAD.GuiUp:
            if obj.ViewObject.DisplayMode == "HiRes":
                ...
                if (
                    ...
                    hires.Placement = obj_place.multiply(obj.CloneOf.Placement).multiply(
                        hires.Placement
                    )
        ...
        if vlist is None:
            ...
            if not m:
                if colors:
                    ...
                    if hasattr(obj.ViewObject, "ShapeAppearance") and hasattr(
    ...
    if materials:
                ...
                if not mat.Name in done:
                    ...
                    for prop in kinds:
                        if prop in mat.Material:
                            outfile.write(
                                kinds[prop]
                                + mat.Material[prop].strip("()").replace(",", " ")
                                + "\n"
                            )
        ...
        FreeCAD.Console.PrintMessage(
            translate("Arch", "Successfully written") + " " + filenamemtl + "\n"
        )
