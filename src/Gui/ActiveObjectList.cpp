...
App::DocumentObject* ActiveObjectList::getObject(
...
) const
{
    ...
    if (subname) {
        ...
        auto sels
            = Gui::Selection().getSelection(_Doc->getDocument()->getName(), ResolveMode::NoResolve);
        for (auto& sel : sels) {
...
)
{
    ...
    if (!info.obj) {
        FC_ERR(
            "Cannot set active object " << obj->getFullName() << '.' << (subname ? subname : "")
