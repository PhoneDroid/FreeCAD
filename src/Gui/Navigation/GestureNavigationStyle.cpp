    ...
    sc::result react(const NS::Event& ev)
    {
        ...
        switch (ns.getViewingMode()) {
            case NavigationStyle::SEEK_WAIT_MODE: {
                if (ev.isPress(1)) {
                    ns.seekToPoint(ev.inventor_event->getPosition());  // implicitly calls
                                                                       // interactiveCountInc()
...
public:
    ...
    sc::result react(const NS::Event& ev)
    {
        ...
        if (ev.isLocation2Event()) {
            // consume all mouse events that Qt fires during the gesture (stupid Qt, so far it only
            // causes trouble)
            ev.flags->processed = true;
        }
...
public:
    ...
    sc::result react(const NS::Event& ev)
    {
        ...
        // roll gestures (same as in AwaitingMoveState, but how to merge them - I don't know
        // --DeepSOIC) direction is determined at the moment the second button is pressed.
        if (ev.mbstate() == 0x101) {
            if (ev.isPress(1)) {
                ns.rollDir = -1;
            }
