 ...
 * Translations:
 *
 * An attempt is made by NotificationArea to translate the message using the "Notifications"
 * context, except for messages which are already translated. Those that are untranslatable can only
 * be send as messages intended for a developer directly using Console..
 *
 * For the former, this may be marked using QT_TRANSLATE_NOOP("Notifications","My message")
 *
 * For translated Notification, many modules using blocking notifications have their translations
 * stored in other contexts, and the translations available at the callee function. This kind of
 * notification provides a very low entry point to move existing blocking notifications into
 * non-intrusive respecting the user choice (given by
 * NotificationArea/NonIntrusiveNotificationsEnabled).
 *
 * Example 1:
 *
 ...
 * notifications, the pop-up will be shown (and it won't appear on the notification area as as a
...
template<
    Base::LogStyle,
    Base::IntendedRecipient = Base::IntendedRecipient::User,
...
inline void Gui::Notify(TNotifier&& notifier, TCaption&& caption, TMessage&& message)
{
    ...
    static_assert(
        recipient != Base::IntendedRecipient::Developer,
        "A Gui notification cannot be intended only for the developer, use Console instead."
    );
    static_assert(
        !(recipient == Base::IntendedRecipient::All && content == Base::ContentType::Translated),
        "Information intended for Developers must not be translated. Provide an untranslated "
        "message instead."
    );
    ...
    if (!nonIntrusive) {
        ...
        if constexpr (content == Base::ContentType::Translated) {
            ...
            // trailing newline is necessary as this may be shown too in a console requiring them
...
{
    Notify<Base::LogStyle::Warning, Base::IntendedRecipient::User, Base::ContentType::Untranslated>(
        std::forward<TNotifier>(notifier),
        std::forward<TCaption>(caption),
        std::forward<TMessage>(message)
    );
...
{
    Notify<Base::LogStyle::Warning, Base::IntendedRecipient::User, Base::ContentType::Translated>(
        std::forward<TNotifier>(notifier),
        std::forward<TCaption>(caption),
        std::forward<TMessage>(message)
    );
...
{
    Notify<Base::LogStyle::Error, Base::IntendedRecipient::All, Base::ContentType::Untranslated>(
        std::forward<TNotifier>(notifier),
        std::forward<TCaption>(caption),
        std::forward<TMessage>(message)
    );
...
{
    Notify<Base::LogStyle::Error, Base::IntendedRecipient::User, Base::ContentType::Untranslated>(
        std::forward<TNotifier>(notifier),
        std::forward<TCaption>(caption),
        std::forward<TMessage>(message)
    );
...
{
    Notify<Base::LogStyle::Error, Base::IntendedRecipient::User, Base::ContentType::Translated>(
        std::forward<TNotifier>(notifier),
        std::forward<TCaption>(caption),
        std::forward<TMessage>(message)
    );
...
{
    Notify<Base::LogStyle::Notification, Base::IntendedRecipient::User, Base::ContentType::Translated>(
        std::forward<TNotifier>(notifier),
        std::forward<TCaption>(caption),
        std::forward<TMessage>(message)
    );
...
{
    Notify<Base::LogStyle::Notification, Base::IntendedRecipient::User, Base::ContentType::Untranslated>(
        std::forward<TNotifier>(notifier),
        std::forward<TCaption>(caption),
        std::forward<TMessage>(message)
    );
