...
void SceneModel::setNode(QModelIndex index, SoNode* node)
{
    ...
    const QString name {
        (it != nodeNames.end()) ? it.value() : QString::fromLatin1(QByteArray(node->getName()))
    };
    this->setData(index.siblingAtColumn(static_cast<int>(Column::NAME)), QVariant(name));

    this->setData(
        index.siblingAtColumn(static_cast<int>(Column::MEMORY_ADDRESS)),
    ...
    if (static_cast<bool>(node->isOfType(SoSwitch::getClassTypeId()))) {
        ...
        this->setData(
            index.siblingAtColumn(static_cast<int>(Column::RENDER_CACHING)),
        ...
        this->setData(
            index.siblingAtColumn(static_cast<int>(Column::BOUNDING_BOX_CACHING)),
            QVariant(
                QString::fromStdString(std::string {formatSoSeparatorCacheEnabled(boundingBoxCaching)})
            )
        );
        ...
        this->setData(
            index.siblingAtColumn(static_cast<int>(Column::BOUNDING_BOX)),
