...
public:
    ...
    /// It shows a dialog with header and message provided and a checkbox in check state with the
    /// message provided. It uses a parameter in path "User parameter:BaseApp/CheckMessages" derived
    /// from the header test, defaulting to false, to store the status of the checkbox, when the
    /// user exits the modal dialog.
    ...
    /// provided, defaulting to entryDefault value if the parameter is not present.
    static void showMessage(
        const QString& header,
        const QString& message,
